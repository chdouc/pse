...
from plot_wave_velocity_fields import row_color_limits
...
def test_load_workflow_remaps_steps_and_validation(tmp_path: Path) -> None:
    ...
    assert output == str(tmp_path / "background_flows" / "data" / "background_flows.npz")
...
def test_repository_spectrum_colormap_is_finite_rgb_table() -> None:
    path = Path(__file__).parents[1] / "code" / "common" / "custom_gradient_32_to_256.mat"
