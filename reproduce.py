...
def portable_command(arguments: list[str]) -> str:
    """Describe an invocation without retaining machine-specific paths."""
    portable = [Path(sys.executable).name, Path(__file__).name]
    path_options = {"--config", "--convergence-config", "--output-directory"}
    index = 0
    while index < len(arguments):
        argument = arguments[index]
        option, separator, value = argument.partition("=")
        if option in path_options and separator:
            portable.append(f"{option}={repository_path(Path(value))}")
        elif argument in path_options and index + 1 < len(arguments):
            portable.extend(
                [argument, repository_path(Path(arguments[index + 1]))]
            )
            index += 1
        else:
...
def portable_message(message: str, paths: list[Path]) -> str:
    """Remove repository and user-directory prefixes from a run message."""
    replacements = [
        (Path(sys.executable), Path(sys.executable).name),
        *((path.resolve(), repository_path(path)) for path in paths),
        (ROOT, "."),
    ]
...
def validate_reusable_simulation(path: Path, config: dict[str, Any]) -> None:
    """Reject a cache generated from different inputs, source or environment."""
    expected_configuration = simulation_signature(config)
    expected_source = simulation_source_signature()
    expected_environment = simulation_environment_signature()
    with h5py.File(path, "r") as handle:
        stored_configuration = handle.attrs.get("simulation_signature_sha256")
        stored_source = handle.attrs.get("simulation_source_signature_sha256")
        stored_environment = handle.attrs.get(
            "simulation_environment_signature_sha256"
        )
    ...
    if stored_source is None:
        raise ValueError(
            "The cached simulation has no solver-source signature; rerun without "
            "--reuse-simulation."
        )
    if stored_source != expected_source:
        raise ValueError(
            "The cached simulation was generated by different solver source; "
            "rerun without --reuse-simulation."
        )
    if stored_environment is None:
        raise ValueError(
            "The cached simulation has no numerical-environment signature; rerun "
            "without --reuse-simulation."
        )
    if stored_environment != expected_environment:
        raise ValueError(
            "The cached simulation was generated in a different numerical "
            "environment; rerun without --reuse-simulation."
        )
...
def write_validation_report(
    ...
    """Atomically replace the current run-status report."""
...
def reproduce_all(
    ...
    *,
    ...
    wave_sidecars = [
        figure_directory / f"sinusoidal_dipole_wave_velocity_{time:g}IP.json"
    ...
    figure_stems = [
        figure_directory / "figure8_error_statistics",
        *[
            figure_directory / f"sinusoidal_dipole_wave_velocity_{time:g}IP"
            for time in config["saved_times_in_inertial_periods"]["figures_9_10"]
        ],
    ]
    figure_outputs = validate_figure_stems(figure_stems)
    validation["figure_outputs"] = {
        "status": "passed",
        "files": [repository_path(path) for path in figure_outputs],
    }
    run_command(
        [
            sys.executable,
...
def main() -> None:
    ...
    write_validation_report(output_directory, validation)
    started = time.perf_counter()
    memory_monitor = PeakMemoryMonitor()
    memory_monitor.start()
    command = portable_command(sys.argv[1:])
    additional_configurations: list[dict[str, str]] = []
    ...
    try:
        ...
        failure = error
        message_paths = [config_path, output_directory]
        if args.convergence_test:
            message_paths.append(args.convergence_config)
        validation = {
            "status": "failed",
            "error_type": type(error).__name__,
            "message": portable_message(str(error), message_paths),
        }
    finally:
    ...
    reused_inputs = (
        [output_directory / "data" / "simulation.h5"] if args.reuse_simulation else []
    )
